...
When meals are enabled for the event, view configured meal periods (or “Meal schedule coming soon” if none are set up yet). When an organizer turns on **Show menu to registrants** for a meal, registrants also see that meal's menu items and dietary tags.
