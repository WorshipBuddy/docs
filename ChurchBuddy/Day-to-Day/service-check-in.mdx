...
**Web:** There is no QR display on the web person profile page. Use the mobile app QR for scanning.

**Mobile app:**
1. From the **Dashboard** (org list), tap **Settings** (⚙️ top right)
2. Tap your **avatar/initials** to open your QR code
3. The code refreshes automatically every **60 seconds** (expires after 2 minutes)

<Info>
The QR code is under **Dashboard → Settings**, not under the org header gear (which opens My Profile).
</Info>
...
- **Quick Results** - See up to **10** matching results
...
Who can check people in depends on organization **People → Check-in** settings:

- **Owner / Organization Admin** — always allowed (all methods)
- **Everyone else** — only if listed in the QR, Matrix, or Manual allowlists (by individual email or team)

Team Admin or Scheduler role alone does **not** grant check-in access unless explicitly allowlisted.
...
Check-in stations are configured under **People → Check-in**, not Organization settings. Each station has:

- **Station ID** - Used in the scanner URL (`?org=&station=`)
- **Station Name** - Display name
- **Location** - Where the station is located (optional)

See [Service Station Scanner](/ChurchBuddy/Day-to-Day/service-station-scanner) for the scanner URL and flow.
