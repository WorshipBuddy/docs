...
You can zoom in/out or enable chords in the bottom center. And if you hold CTRL/CMD and scroll up or down over the slides in the left panel you can show more slides at once.
...
Shrink to fit: Text is set font size by default, but can shrink if the text does not fit in the textbox. (default)
Grow to fit: Text will grow to fill the entire textbox, but maximum the set font size.
## Text scrolling
...
## Modes
At the bottom you can change the edit mode, first you can click on "Chords" to add chords. Also you can toggle "Text edit".

## Text edit
This works in a similar way to the "Quick lyrics" when creating a show, or it is the same if it is empty. It will automatically change slides in the current layout based on the content of the text after editing. It will update the content when the textarea loses focus (If you press esc, or click on another area). Useful for copying/pasting song lyrics, and for splitting/joining slides.